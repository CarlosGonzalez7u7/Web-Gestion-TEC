<?php
// php/bitacora/actualizar_requisitos_semestre.php

header('Content-Type: application/json');
require '../conexion.php';

$ID_semestre = isset($_POST['ID_semestre']) ? (int) $_POST['ID_semestre'] : 0;
$docentes    = isset($_POST['docentes'])
               ? json_decode($_POST['docentes'], true)
               : [];
$requisitos  = isset($_POST['requisitos'])
               ? json_decode($_POST['requisitos'], true)
               : [];
...
try {
    // 1) Leer todas las filas existentes para este semestre
    $sqlSel = "
    ...
    // 2) Construir el conjunto de combos deseados tras la edición
    $desired = [];
    ...
    // 3) Eliminar las filas que EXISTÍAN pero ya NO están en desired
    ...
    // 4) Insertar SOLO los combos que están en desired pero NO en existing
    ...
    echo json_encode([
      'success'=>false,
      'message'=>'Error al actualizar requisitos: '.$e->getMessage()
    ]);
}
$conexion->close();
